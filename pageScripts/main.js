...
let ajax_interceptor_jd = {
  ...
  myXHR: function() {
    ...
    const xhr = new ajax_interceptor_jd.originalXHR;
    for (let attr in xhr) {
      if (attr === 'onreadystatechange') {
        xhr.onreadystatechange = (...args) => {
          if (this.readyState == 4) {
            // 请求成功
            if (ajax_interceptor_jd.settings.ajaxInterceptor_switchOn) {
              // 开启拦截
              modifyResponse();
            }
          }
  ...
  myFetch: function(...args) {
    return ajax_interceptor_jd.originalFetch(...args).then((response) => {
      let txt = undefined;
      ajax_interceptor_jd.settings.ajaxInterceptor_rules.forEach(({filterType = 'normal', switchOn = true, match, overrideTxt = ''}) => {
        let matched = false;
        if (switchOn && match) {
...
window.addEventListener("message", function(event) {
  ...
  if (data.type === 'ajaxInterceptor' && data.to === 'pageScript') {
    ajax_interceptor_jd.settings[data.key] = data.value;
  }

  if (ajax_interceptor_jd.settings.ajaxInterceptor_switchOn) {
    window.XMLHttpRequest = ajax_interceptor_jd.myXHR;
    window.fetch = ajax_interceptor_jd.myFetch;
  } else {
    window.XMLHttpRequest = ajax_interceptor_jd.originalXHR;
    window.fetch = ajax_interceptor_jd.originalFetch;
  }
}, false);
